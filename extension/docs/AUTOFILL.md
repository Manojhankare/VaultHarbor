...
4. If the vault is **locked**, the fill icon still appears on detected login fields. The picker shows **VaultHarbor is locked** and **Unlock**. If the user is **signed out**, the same overlay says **You're not signed in** / **Sign in**. If they are signed in but have not created a master password yet, it says **Finish vault setup**. Unlock / Sign in / Set up opens the toolbar popup or a small `popup.html?unlock=1` window — the master password is not typed in the page overlay. After sign-in or unlock, the overlay refreshes to matching logins. Unlocked pages with no saved login still hide the icon.
5. Detection walks open/closed shadow roots (`chrome.dom.openOrClosedShadowRoot`) so custom-element fields (Workday) are found. Overlay attachment also walks shadow hosts to find the page form/dialog. Autofill remembers the focused field if a full-page scan misses the modal. The fill icon uses `isConnected` (not `document.contains`) so shadow-tree fields are not treated as detached.
...
If the vault is **locked** when the save iframe opens, the UI shows **Unlock to save** first (master password + **Keep unlocked this session**, checked by default). After unlock it advances to the editable save form. **Keep unlocked** skips auto-lock for the rest of the browser session (cleared on manual Lock / Logout / browser restart). The same default applies on the popup unlock screen. Configure the idle timeout under **Security → Auto-lock** in the full vault app.
